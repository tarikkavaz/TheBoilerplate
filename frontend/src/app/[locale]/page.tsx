import { Homepage, MetadataProps, HomeProps, ContentImage } from "@/utils/types";
...
export async function generateMetadata(
  ...
  const firstImageUrl = firstPost.images && firstPost.images[0] ? firstPost.images[0].image : DEFAULT_OG_IMAGE_URL;
  ...
  return (
    <Container className="p-10 mt-16" id="content">
      ...
      <div className="flex-wrap hidden gap-4 p-4 my-16 rounded bg-slate-300">
        ...
        <Button className="" variant="secondary">{t("button")}</Button>
        <Button className="" variant="destructive">{t("button")}</Button>
        <Button className="" variant="outline">{t("button")}</Button>
        <Button className="" variant="ghost">{t("button")}</Button>
        <Button className="" variant="link">{t("button")}</Button>
        <Button>
